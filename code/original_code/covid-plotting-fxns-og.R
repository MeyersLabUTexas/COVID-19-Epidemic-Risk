...
plot_county_risk <- function(county_data, state = NULL){
  ...
  shp_file %>% 
    ...
    scale_fill_gradient(low = "gainsboro", high = "dark red", name = "Chance (%)",
                        breaks=c(8.936, 25, 50, 75, 100), labels=c(9, 25, 50, 75, 100), limits=c(8.936,100))+
    scale_x_continuous("", breaks = NULL) + scale_y_continuous("", breaks = NULL)+
    theme(panel.background = element_rect(color = "white", fill = "white"),
          legend.title=element_text(size=10),
          legend.text=element_text(size=8))+
    guides(fill = guide_colourbar(barheight = unit(2, "in"), ticks.colour='black', ticks.linewidth = 1,
...
### Fig 2
make_case_risk_plot=function(r_not_vect, det_prob, dir_path, fig_path, date1="2020-03-16", date2="2020-03-23",
                             county_ex="Burnet County", state_ex="Texas"){
  ...
  # Open files with county epi_prob per date
  date1_county = read_csv(paste0(dir_path, "/", date1, "county-risk-estimates.csv")) %>%
    filter(state==state_ex) %>%
    filter(county==county_ex)
  date2_county = read_csv(paste0(dir_path, "/", date2, "county-risk-estimates.csv")) %>%
    filter(state==state_ex) %>%
    filter(county==county_ex)
...
epi_risk_pop_cor_plot = function(dir_path, fig_path, date1="2020-03-16", date2="2020-03-23"){
  ...
  # Rural and Underserved counties 2019
  # https://www.consumerfinance.gov/compliance/compliance-resources/mortgage-resources/rural-and-underserved-counties-list/
  rural_underserve = read_csv("raw_data/cfpb_rural-underserved-list_2019.csv") %>%
    rename(fips = `Fips Code`) %>%
    mutate(rural_underserve = 1) %>%
    select(fips, rural_underserve)

  
  # Open files with county epi_prob per date
  county_dates_df = read_csv(paste0(dir_path, "/", date1, "county-risk-estimates.csv")) %>%
    rename(date1     = date, cases1    = cases, deaths1   = deaths, epi_prob1 = epi_prob) %>%
    left_join(read_csv(paste0(dir_path, "/", date2, "county-risk-estimates.csv") ), 
  ...
  sub_county_dates_df  = county_dates_df %>%
